<x-app-layout>
    <div class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <!-- Breadcrumb -->
            <div class="mb-6">
                <a href="{{ route('admin.maintenance.show', $maintenance) }}" class="text-indigo-600 hover:text-indigo-900">
                    ← Back to Maintenance Details
                </a>
            </div>

            <!-- Header -->
            <h2 class="text-2xl font-bold text-gray-900 mb-6">Edit Maintenance Request</h2>

            <!-- Form -->
            <form method="POST" action="{{ route('admin.maintenance.update', $maintenance) }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                        ...
                        <div>
                            ...
                            <select name="lease_id"
                                ...
                                @foreach($leases as $lease)
                                    <option value="{{ $lease->id }}" {{ old('lease_id', $maintenance->lease_id) == $lease->id ? 'selected' : '' }}>
                ...
                <div class="bg-white rounded-lg shadow-sm p-6">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="desc"
                                  ...
                                  class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('desc') border-red-500 @enderror">{{ old('desc', $maintenance->desc) }}</textarea>
                        @error('desc')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label for="photo_path" class="block text-sm font-medium text-gray-700 mb-1">
                            {{ $maintenance->photo_path ? 'Replace Photo' : 'Upload Photo' }} (Optional)
                        </label>
                        <input type="file"
                               name="photo_path"
                               id="photo_path"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <input type="number"
                                   name="cost"
                                   id="cost"
                                   value="{{ old('cost', $maintenance->cost / 100) }}"
                                   step="0.01"
                                   min="0"
                                   required
                                   placeholder="0.00"
                        ...
                        <div>
                            ...
                            <select name="paid_by"
                                ...
                                @foreach($paidByOptions as $paidByOption)
                                    <option value="{{ $paidByOption }}" {{ old('paid_by', $maintenance->paid_by) == $paidByOption ? 'selected' : '' }}>
                                        {{ $paidByOption }}
                                    </option>
                                @endforeach
                            </select>
    ...
    <script>
        ...
        document.getElementById('lease_id').addEventListener('change', function() {
            ...
            const currentAssetId = '{{ old('asset_id', $maintenance->asset_id) }}';
            ...
            fetch(`/admin/maintenance/assets-by-lease?lease_id=${leaseId}`)
                ...
                .then(assets => {
                    ...
                    assets.forEach(asset => {
                        ...
                        // Re-select the current asset if it's in the list
        ...
        // Trigger asset loading on page load to populate assets for current lease
