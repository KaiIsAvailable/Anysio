<div class="space-y-6"> {{-- 容器，给每个卡片一点间距 --}}
    @forelse($payments as $payment)
        <div x-data="{ showModal: false, loading: false }" @submit="loading = true" class="mb-6">
                    ...
                    <div class="flex-1 flex items-center gap-4 md:gap-6 p-6 md:p-8">
                        <div class="relative group cursor-pointer shrink-0" @click="showModal = true">
                            ...
                            <div class="relative h-16 w-16 md:h-20 md:w-20 overflow-hidden rounded-xl border-[3px] border-white shadow-lg">
                                ...
                                <div class="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all duration-300">
                                    <svg class="w-6 h-6 text-white transform scale-50 group-hover:scale-100 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="min-w-0 flex-1 space-y-1">
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest truncate">(REF:{{ $payment->transaction_ref }})</span>
                            </div>
                    ...
                    <div class="md:w-64 bg-slate-50/80 border-t md:border-t-0 md:border-l border-slate-100 p-4 md:p-6 flex flex-row md:flex-col justify-center items-center gap-3">
                        ...
                        <x-primary-button type="button" 
                            @click="$dispatch('open-payment-record', { 
                                ...
                                actionUrl: '{{ route('admin.payments.approve', $payment->id) }}'
                            })"
                            ...
                            <span class="flex items-center justify-center gap-2" loading="loading">
                                Approve 
            ...
            <template x-teleport="body">
                <div x-show="showModal" x-cloak class="fixed inset-0 z-[999] overflow-y-auto">
                    <x-ui.blur-overlay show="showModal" onClose="showModal = false" zIndex="z-[998]" />

                    <div class="flex items-center justify-center min-h-screen px-4 py-12">
                        <div class="relative z-[1000] bg-white rounded-[2.5rem] shadow-2xl max-w-2xl w-full overflow-hidden transition-all duration-300"
                             x-show="showModal"
                            ...
                            <div class="p-4 bg-slate-100 flex items-center justify-center">
                                <img src="{{ route('admin.receipt.display', $payment->attachment) }}" class="max-h-[60vh] w-auto object-contain rounded-2xl shadow-sm border-4 border-white">
                            </div>
