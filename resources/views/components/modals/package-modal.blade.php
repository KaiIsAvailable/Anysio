        ...
        <div class="relative bg-white rounded-[2rem] shadow-2xl w-fit min-w-[320px] overflow-hidden z-[102]"
            ...
            <div class="px-6 pb-6 space-y-4">
                @if(auth()->check() && auth()->user()->role === 'admin')
                    {{-- Admin view message --}}
                    <div class="bg-amber-50 border border-amber-200 p-6 rounded-2xl text-center space-y-2 min-w-[280px]">
                        <div class="text-amber-600 font-bold text-base">
                            You are admin, you have no package
                        </div>
                        <p class="text-xs text-amber-500">
                            System administrators have unrestricted access and do not require a subscription package.
                        </p>
                    </div>
                @else
                    {{-- Regular package details view for non-admins --}}
                    {{-- 主要卡片 --}}
