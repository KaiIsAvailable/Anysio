...
<template x-teleport="body">
    <div x-show="openPayment" 
        ...
        <div class="flex items-center justify-center min-h-screen px-4 py-6">
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden transition-all duration-200"
                 x-show="openPayment"
                ...
                {{-- ### 关键修改：在这里添加 x-data ### --}}
                <form :action="paymentData.actionUrl" method="POST" x-data="{ method: '01' }">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select name="received_via" 
                                        ...
                                        required 
                                        class="block w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 text-sm">
                                    <option value="01">Cash (01)</option>
                                    <option value="02">Cheque (02)</option>
                                    <option value="03">Bank Transfer (03)</option>
                                    <option value="04">Credit Card (04)</option>
                                    <option value="05">Debit Card (05)</option>
                                    <option value="06">e-Wallet / Digital Wallet (06)</option>
                                    <option value="07">Digital Bank (07)</option>
                                    <option value="08">Others (08)</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="mt-4 space-y-4">
                            {{-- Cheque --}}
                            <div x-show="method === '02'" x-transition x-cloak>
                            ...
                            <div x-show="['03', '06', '07'].includes(method)" x-transition x-cloak>
                                ...
                                <input type="text" name="transaction_ref" 
                                    :required="['03', '06', '07'].includes(method)"
                                    placeholder="e.g. MYR88231... or TNG-992..."
                                    class="block w-full px-4 py-3 bg-white border border-indigo-200 rounded-xl focus:ring-2 focus:ring-indigo-500 text-sm shadow-sm">
                            </div>

                            {{-- Cards --}}
                            <div x-show="['04', '05'].includes(method)" x-transition x-cloak>
                                ...
                                <input type="text" name="transaction_ref" 
                                    :required="['04', '05'].includes(method)"
                                    placeholder="e.g. 8890"
                                    class="block w-full px-4 py-3 bg-white border border-indigo-200 rounded-xl focus:ring-2 focus:ring-indigo-500 text-sm shadow-sm">
                            </div>

                            {{-- Cash --}}
                            <div x-show="method === '01'" x-transition x-cloak class="p-3 bg-amber-50 rounded-lg border border-amber-100">
                                ...
                                <input type="hidden" name="transaction_ref" value="CASH">
                            </div>
                        </div>
